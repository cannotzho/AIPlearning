...
The vector search function didn't take very long to implement. But now there's a big problem - why's the accuracy so bad?!?! the distance between "cow" and "person" is 0 - what the heck is going on? It leads me to believe that maybe I'm not ordering vectors in the KDTree object properly, which leads to wrong indexing between vectors and their actual indices. I'm gonna switch to brute force first to see if this is the case then move on from there.

Turns out my code was just bad, missed out that I was returning only indices and not dists, yet i was searching for k = 2 instead of k = 1 like i thought, and just being confused by what was being returned in general. Now that the search for the keywords work, it's actually pretty amazing! I just have to pass the suggested keywords through the mapping database and then the search functionality is done (for search by detected objects at least). If I want to include by video filename as well... then i have to encode the filenames into the embedded space during upload also. Maybe I will implement that at a later stage, seems kinda extra lol.

Completed search functionality... Now I can do up the health endpoint and format the html abit, kinda tired of looking at all the stuff being pushed to the left. It works but sadly because the object detection is pretty inaccurate, the traffic video is giving dining tables and boats instead of cars, which makes the search results kind of unexpected also. I'm gonna leave this stuff to after if there's time, after the health endpoint its frontend, then i'll look into the whole containerization thing, then testing.

React js is pretty amazing! I'm glad that I get to try development work in 2025 and not like, 2012 or something. So, following are my tasks:
_ File upload interface for single/batched video file processing.
_ Display a list of all processed videos from the database.
_ Show extracted key frames with detected objects highlighted.
_ Implement search functionality to search for videos based on:
_ Text queries (matching against object labels and video filenames)
_ Visual similarity (by selecting an existing video/frame as reference)

I'll start by drawing out the basic component tree for the react js app. For some of these features like the extracted key frames, I probably need to create new tables in my database which means updating the backend. Although I have the rough gist of how to do the frontend app, I still need to figure out how to link up the backend endpoints to the frontend UI components, or buttons I guess. This should just be equal to passing the backend url as a form action in the frontend?

Bootstrap has lots of ready-made components I can use so I'm just going to piggyback off that. Since this is a single-page application, it should fit the upload feature, list of videos, search and extracted key frames all in one place. The upload and extracted key frames can be popup windows, maybe. Oh - actually only the upload, extracted keyframes could take up a window on the right as the main viewing, and then only show up when a video is selected.

Damn after a lot of troubleshooting I actually managed to configure the proxy of the frontend and connect it to the backend port. But as I write the frontend components to accept the JSON responses from the backend, I feel like its becoming kinda hard to type safety for Props while keeping components as flexible as possible. It's pretty tedious but it looks like I have to rewrite some of this and also, I need to modify the backend db structure if I want to display the processed keyframes and search by filename...

Got the upload button working and the grid display is type specified now... but I guess it's less reusable as a component. It's time to return to the backend and see if i can start saving keyframes while processing the video

Managed to get the keyframe saving working but it's a huge pain in the butt because i messed up at the start... my flask app env is outside of my backend folder and running the script from the folder housing both the frontend and backend is causing a lot of grief with path management within the flask app. I gotta see if I can fix this at some point by moving the env inside somehow. I probably have to do this when I package everything towards the end anyway so I guess I'll first work on the filename search.

For the filename search, I'm thinking if it makes more sense to just rely on conventional SQL LIKE queries since video filenames may not always consist of actual words. Seems like a pain to go vectorize the filenames as well. Actually... if i treat it as a keyword it might not be that bad? After finding nearest neighbor in keywords, i try to search for that keyword within the uri column of the video database instead of going through the relational table? Could be good.

Finally all the functionality pretty much works. I will do the containerization and packaging so that the image can be run consistently by other machines. After that, i can work on the documentation and readme then I should be done!

Using docker seems pretty straightforward so far, but waiting for each build takes some time. Getting requirements.txt from pip freeze alone doesn't seem sufficient as well because there are other dependencies outside of python libraries like compilers and stuff, making the troubleshooting process kind of annoying. I don't think i have any other way apart from just building, trying to run the server and then updating the Dockerfile each time i encounter some import error lol. Also after showing my bro the app he pointed out that i didn't display the videos themselves at all which is true, seems like i missed a pretty critical part of the app by just displaying the video names only TT. Thankfully adding that feature wasn't too complicated, just had to edit the endpoint responses. I think it makes more sense also that the Video resource is returning an actual video now, and the list of keyframes comes from a Keyframe endpoint.
